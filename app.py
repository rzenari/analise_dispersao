...
def carregar_dados(arquivo_enviado):
    ...
    # Padronização dos nomes das colunas para evitar erros de 'não encontrado'
    df.columns = df.columns.str.lower().str.strip()

    # Verificação das colunas essenciais
    if 'latitude' not in df.columns or 'longitude' not in df.columns:
        st.error("ERRO CRÍTICO: As colunas 'latitude' e/ou 'longitude' não foram encontradas. Verifique o arquivo de origem.")
        st.write("Colunas encontradas (após padronização):", df.columns.tolist())
        return None
    ...
    try:
        ...
        st.error(f"Erro ao converter as colunas de latitude/longitude. Verifique se elas contêm apenas valores numéricos. Detalhe: {e}")
        return None
